...
class Trans_po_edit extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->database();
		$this->load->model('Model_trans_po_edit');
	...
	public function index($id)	{
		...
		$data_all = $this->Model_trans_po_edit->get_data($id);
		$data['data_all'] = $data_all;

		$data['get_list_enum'] = $this->Model_trans_po_edit->get_list_enum('trans_po', 'type');
		
		$dd_material = $this->Model_trans_po_edit->get_list_material();
		$data['dd_material'] = $dd_material;
		
		$dd_storage = $this->Model_trans_po_edit->get_list_storage();
		$data['dd_storage'] = $dd_storage;
		
		$dd_movement_reason = $this->Model_trans_po_edit->get_list_movement_reason();
		$data['dd_movement_reason'] = $dd_movement_reason;
		
		$dd_vendor = $this->Model_trans_po_edit->get_list_vendor();
		$data['dd_vendor'] = $dd_vendor;
                
                $dd_barge = $this->Model_trans_po_edit->get_list_barge();
                $data['dd_barge'] = $dd_barge;
                
                $dd_transporter = $this->Model_trans_po_edit->get_list_transporter();
                $data['dd_transporter'] = $dd_transporter;
                
                
		
		$this->load->view('header', $datasesion);
		$this->load->view('trans_po_edit', $data);
	...
	public function simpan($id)	{
		...
		if ($this->form_validation->run() == FALSE){
			...
			$trans_id = $this->input->post('trans_id');
			if ($this->Model_trans_po_edit->update_data($data, $trans_id)){
                                ...
                                $id = $this->input->post('po_res_number');
                                $data_schedule = array(
                                    'barge_id' => $this->input->post('barge_id'),
                                ...
                                if($this->Model_trans_po_edit->update_barge_schedule($data_schedule, $id)){
