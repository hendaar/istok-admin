...
class Model_cleanliness_add extends CI_Model {
	...
	function get_cleanliness($id){
		$this->db->where('cleanliness_name', $id);
        $query = $this->db->get('mst_cleanliness');
		return $query->result();
	}
	
	function insert_cleanliness($data){
		return $this->db->insert('mst_cleanliness', $data);
	}
